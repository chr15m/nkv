...
echo "$output"
if ! [ -f ./.nkv ]; then
  printf "%b\n" "${FAIL} .nkv file not created."
  exit 1
fi
if ! echo "$output" | grep -q "Wrote config to .nkv"; then
  printf "%b\n" "${FAIL} Did not get expected output for --init."
  exit 1
fi
printf "%b\n" "${PASS} --init created .nkv file."
...
if ! echo "$output" | grep -q "Config already present in .nkv"; then
  printf "%b\n" "${FAIL} Did not get expected output for existing .nkv."
  exit 1
fi
printf "%b\n" "${PASS} --init failed as expected."
rm ./.nkv
...
if [ "$output" != "myvalue" ]; then
  printf "%b\n" "${FAIL} Expected 'myvalue', got '$output'."
  exit 1
fi
printf "%b\n" "${PASS} Set and get successful."
rm ./.nkv
...
if [ "$output" != "envvalue" ]; then
  printf "%b\n" "${FAIL} Expected 'envvalue', got '$output'."
  exit 1
fi
if [ -f ./.nkv ]; then
  printf "%b\n" "${FAIL} .nkv file was created when using env vars."
  rm ./.nkv
  exit 1
fi
printf "%b\n" "${PASS} Environment variables successful."

echo "--- Test: --help flag ---"
output=$(${binary} --help 2>&1)
exit_code=$?
if [ $exit_code -ne 0 ]; then
  printf "%b\n" "${FAIL} --help exited with code $exit_code, expected 0."
  exit 1
fi
...
printf "%b\n" "${PASS} --help flag works as expected."

echo "--- Test: Read non-existent key ---"
output=$(${binary} non_existent_key 2>&1)
exit_code=$?
if [ $exit_code -ne 1 ]; then
  printf "%b\n" "${FAIL} Reading non-existent key exited with code $exit_code, expected 1."
  exit 1
fi
if ! echo "$output" | grep -q "No value found for key"; then
  printf "%b\n" "${FAIL} Did not get expected output for non-existent key."
  exit 1
fi
printf "%b\n" "${PASS} Reading non-existent key failed as expected."
...
if [ "$watch_output" != "updated" ]; then
  printf "%b\n" "${FAIL} --watch did not produce the correct output. Expected 'updated', got '$watch_output'."
...
printf "%b\n" "${PASS} --watch functionality works as expected."
rm "$WATCH_FILE" watch-set1.log watch-set2.log watch-stderr.log
